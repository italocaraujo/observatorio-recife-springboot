...
public class PibController {
    ...
    private static final String BASE_PATH = "data-json/pib/anos";
    ...
    @GetMapping("/{ano}")
    public ResponseEntity<?> getPibPorAno(@PathVariable String ano) {
        return loadResponse(String.format("%s/PIB_municipios_%s.parquet", BASE_PATH, ano), false);
    }

    @GetMapping("/{ano}/processed")
    public ResponseEntity<?> getPibPorAnoProcessed(@PathVariable String ano) {
        return loadResponse(String.format("%s/PIB_municipios_%s.parquet", BASE_PATH, ano), true);
    }
}
